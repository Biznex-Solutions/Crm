...
@section('page-title', 'Create New User Profile')
...
<div class="form-card">
    <div class="form-header-title">
        <h2><i class="fa-solid fa-user-plus" style="color: var(--accent); margin-right: 8px;"></i> Create New System User</h2>
        <p>Fill out the required information to register a new admin or employee user into Biznex CRM.</p>
    </div>
    ...
    <form action="{{ route('users.store') }}" method="POST">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-input" value="{{ old('name') }}" placeholder="e.g. Ali Raza" required>
            ...
            <div class="form-group">
                ...
                <input type="email" id="email" name="email" class="form-input" value="{{ old('email') }}" placeholder="ali@biznex.com" required>
            ...
            <div class="form-group">
                ...
                <input type="text" id="phone" name="phone" class="form-input" value="{{ old('phone') }}" placeholder="+92 300 1234567">
            ...
            <div class="form-group">
                ...
                <input type="text" id="designation" name="designation" class="form-input" value="{{ old('designation') }}" placeholder="e.g. Sales Partner / Consultant">
            ...
            <div class="form-group full-width">
                ...
                <div class="role-options-grid">
                    <label class="role-card-option {{ old('role', 'employee') == 'employee' ? 'selected' : '' }}" onclick="selectRole(this)">
                        <input type="radio" name="role" value="employee" {{ old('role', 'employee') == 'employee' ? 'checked' : '' }} required>
                        <div class="role-info">
                    ...
                    <label class="role-card-option {{ old('role') == 'admin' ? 'selected' : '' }}" onclick="selectRole(this)">
            ...
            <div class="form-group full-width" id="categoryTargetWrapper" style="{{ old('role', 'employee') === 'admin' ? 'display:none;' : '' }}">
                ...
                <div class="category-checkbox-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 10px; max-height: 260px; overflow-y: auto; padding: 12px; background: #FAFBFF; border: 1.5px solid var(--border); border-radius: 14px;">
                    @foreach($categories as $category)
                        <label class="cat-check-item" style="display: flex; align-items: center; gap: 10px; padding: 10px 12px; background: #FFFFFF; border: 1px solid var(--border); border-radius: 10px; cursor: pointer; transition: all 0.15s;">
                            <input type="checkbox" name="category_target_ids[]" value="{{ $category->id }}" class="cat-checkbox" {{ (is_array(old('category_target_ids')) && in_array($category->id, old('category_target_ids'))) ? 'checked' : '' }} style="accent-color: var(--accent); width: 16px; height: 16px; flex-shrink: 0;">
            ...
            <!-- Account Status -->
            <div class="form-group full-width">
                <label class="form-label" for="status">Account Initial Status <span style="color: var(--danger);">*</span></label>
                <select id="status" name="status" class="form-select" required>
                    <option value="active" {{ old('status', 'active') == 'active' ? 'selected' : '' }}>Active (User can sign in immediately)</option>
                    <option value="inactive" {{ old('status', 'inactive') == 'inactive' ? 'selected' : '' }}>Inactive (Access blocked until activated)</option>
                </select>
