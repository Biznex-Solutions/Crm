...
@section('title', 'Edit Category Target')
@section('page-title', 'Edit Category Target')
...
<div class="form-card">
    <div class="form-header">
        <h2>
            ...
            <span>Edit Category Target: {{ $categoryTarget->name }}</span>
        </h2>
    ...
    <form action="{{ route('category-targets.update', $categoryTarget) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-body">
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-input" value="{{ old('name', $categoryTarget->name) }}" required>
                @error('name')
            ...
            <div class="form-grid-2" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div class="form-group">
                    ...
                    <input type="number" id="target_deals" name="target_deals" class="form-input" value="{{ old('target_deals', $categoryTarget->target_deals) }}" min="0" required>
                    @error('target_deals')
                        <div class="error-text"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-select" required>
                        <option value="active" {{ old('status', $categoryTarget->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $categoryTarget->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <div class="error-text"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                    @enderror
            ...
            <div class="form-group" style="margin-bottom: 0;">
                ...
                <textarea id="description" name="description" class="form-textarea">{{ old('description', $categoryTarget->description) }}</textarea>
                @error('description')
                    <div class="error-text"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-footer">
            ...
            <button type="submit" class="btn btn-primary">
                ...
                <span>Update Category Target</span>
            </button>
        </div>
    </form>
</div>
